...
export class KMeansService {
  ...
  private getOrderCoords(order: any): { lat: number; lng: number } {
   ...
   */
  public clusterOrders(orders: any[], k: number): Cluster[] {
  ...
  private runCapacityKMeans(validOrders: any[], k: number): Cluster[] {
  ...
  private calculateClusterLoad(orders: any[]): { weight: number; volume: number } {
    let totalWeight = 0;
    let totalVolume = 0;
