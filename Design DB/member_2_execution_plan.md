...
### BƯỚC 1: BỘ TÍNH PHÍ NHANH (QUICK PRICING CALCULATOR)
*   **Mục tiêu:** Cho phép khách hàng vãng lai ước tính phí vận chuyển ngay tại Landing Page.
*   **Yêu cầu kỹ thuật:**
    *   Tạo file `src/components/PricingCalculator.tsx`.
    *   Giao diện thiết kế theo phong cách Glassmorphism sang trọng.
    *   Các tham số đầu vào: Khoảng cách (km), Trọng lượng (kg), Gói dịch vụ (`EXPRESS`, `STANDARD`, `SAVING`, `COLD_CHAIN`), Tùy chọn (Bảo hiểm hàng hóa, Khai giá trị COD, Hàng dễ vỡ).
    *   Áp dụng đúng công thức tính phí từ Backend để hiển thị kết quả trực quan (từng loại phí chi tiết).

### BƯỚC 2: KẾT NỐI API AUTH & PHÂN QUYỀN ĐỘNG (RBAC)
*   **Mục tiêu:** Thay thế dữ liệu Auth Mock bằng API thật kết nối tới Backend.
*   **Yêu cầu kỹ thuật:**
    *   Cập nhật `AuthModal.tsx` để gửi request đăng nhập/đăng ký tới backend.
    *   Khi đăng nhập thành công, lưu Token vào LocalStorage.
    *   Đọc thông tin profile và danh sách quyền hạn (permissions) hiển thị lên giao diện quản trị.
    *   Bảo vệ dashboard bằng kiểm tra vai trò: chỉ `ADMIN`, `STAFF` hoặc `CUSTOMER` mới hiển thị các tab/nhiệm vụ tương ứng.

### BƯỚC 3: XÂY DỰNG BẢNG ĐIỀU KHIỂN QUẢN TRỊ (ADMIN DASHBOARD)
*   **Mục tiêu:** Cung cấp giao diện quản trị toàn diện cho nội bộ (Staff/Admin) và Khách hàng.
*   **Yêu cầu kỹ thuật:**
    *   Tạo file `src/features/dashboard/components/AdminDashboard.tsx` tích hợp Sidebar/Tabs linh hoạt.
    *   **Tab Đơn hàng (Order Management):**
        *   Hiển thị danh sách toàn bộ đơn hàng trong hệ thống (phân trang/lọc trạng thái).
        *   Chi tiết đơn hàng: Timeline trạng thái thực, thông tin Snapshot lúc tạo, bản đồ trực quan.
        *   Chức năng cập nhật trạng thái đơn hàng (Dành cho `STAFF`/`ADMIN`).
    *   **Tab Khách hàng (Customer Management):**
        *   Hiển thị danh sách khách hàng, phân loại Loại khách hàng (Individual/Business).
        *   Xem chi tiết thông tin và sổ địa chỉ đã lưu (gồm các địa chỉ có `wardCode` chuẩn hóa).
    *   **Tab Kho bãi (Facility Management):**
        *   Hiển thị danh sách kho bãi (Tổng kho, Hub, Micro Hub).
        *   Tạo mới Kho bãi (có form nhập địa chỉ và tìm kiếm tọa độ/wardCode).
        *   Xem phân khu lưu kho (Cargo Zones) của từng kho bãi và thêm phân khu mới.
