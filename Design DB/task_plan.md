...
> **Mục tiêu:** Chuẩn hóa quy định mỗi Tỉnh/Thành phố có **1 Kho tổng (Provincial Hub)** làm đầu mối, các Bưu cục nhỏ trong tỉnh đóng vai trò là **Trạm giao nhận (Station / Sub-hub)**.

* **Cấu hình bảng `facilities`:**
  * Thêm/Chuẩn hóa liên kết `province_code` (FK ➔ `provinces.code`) để xác định bưu cục thuộc Tỉnh/Thành nào.
  * Phân biệt rõ loại bưu cục `facility_type`: `PROVINCIAL_HUB` (Kho tổng Tỉnh), `DISTRICT_STATION` (Trạm bưu cục Huyện/Quận).
  * Quy tắc cây phân cấp `parent_facility_id`: Các trạm con trong tỉnh trỏ `parent_facility_id` về đúng Kho tổng của Tỉnh đó.

---

### Phase 4: Tái cấu trúc Bảng `packages` (Bao tải/Sọt hàng gom nhiều đơn)
> **Mục tiêu:** Thống nhất định nghĩa `package` là một **Bao hàng / Sọt chứa (Container/Bag)** gom từ nhiều đơn hàng (`orders`) để trung chuyển.

* **Điều chỉnh mô hình:**
  * Chuyển đổi mối quan hệ từ `Order 1-N Package` thành mô hình Gom hàng (`Package` là Bao/Sọt chứa danh sách `Order` hoặc ngược lại).
  * Cập nhật các bảng liên kết `shipment_packages` và quy trình quét barcode kiểm đếm kho.
